...
class Manager {
    ...
    protected function _arrayDiffRecursive($firstArray, $secondArray) {
        ...
        foreach (array_keys($secondArray) as $key) {
            $secondArray[$key] = $secondArray[$key] instanceof PagosPymeEntity ? $secondArray[$key]->toArray() : $secondArray[$key];
            if (array_key_exists($key, $firstArray) && $firstArray[$key] instanceof PagosPymeEntity) {
    ...
    protected function _setDefaultHeaders(&$query) {
        ...
        $query['headers']['User-Agent'] = 'PagosPyme DX-PHP SDK/ v' . Version::$_VERSION;
