...
class MetaDataReader {
    ...
    public function getMetaData($entity) {
        ...
        foreach ($classAnnotations as $annotation) {
            if ($annotation instanceof \PagosPyme\Annotation\RestMethod) {
                $result->methods[$annotation->method] = get_object_vars($annotation);
            }
            if ($annotation instanceof \PagosPyme\Annotation\RequestParam) {
                $result->params[] = $annotation->param;
            }
            if ($annotation instanceof \PagosPyme\Annotation\DenyDynamicAttribute) {
                $result->denyDynamicAttribute = true;
            }
        }

        foreach ($propertyAnnotations as $key => $annotation) {
            if ($annotation instanceof \PagosPyme\Annotation\Attribute) {
